...
void BuildingPlacer::init() {
  ...
  for (BWAPI::TilePosition pos(0, 0); pos.x < mapWidth; ++pos.x)
    for (pos.y = 0; pos.y < mapHeight; ++pos.y)
      mapCanBuild[pos.x + pos.y * 256] =
          BWEM::Map::Instance().GetTile(pos).Buildable();
}

BWAPI::TilePosition
BuildingPlacer::getBuildLocation(BWAPI::UnitType ut,
                                 BWAPI::TilePosition suggestedPosition) {
  if (ut.getRace() == BWAPI::Races::Zerg && !ut.isBuilding())
    ...
    for (int y = 0; y < mapHeight; ++y) {
      if (needsCreep && BWAPI::Broodwar->hasCreep(BWAPI::TilePosition(x, y)))
        ...
        canBuild[x + y * 256] = false;
               ...
               !BWAPI::Broodwar->hasCreep(BWAPI::TilePosition(x, y)))
        canBuild[x + y * 256] = mapCanBuild[x + y * 256];
      else if (!canBePlacedOnCreep)
        canBuild[x + y * 256] = false;
  ...
  for (auto &b : BWAPI::Broodwar->getAllUnits()) {
    if ((b->getType().isBuilding() || b->getType().isResourceContainer()) &&
        !b->isFlying()) {
      unsigned x = b->getTilePosition().x,
               xEnd = b->getTilePosition().x + b->getType().tileWidth();
      unsigned y = b->getTilePosition().y,
               yEnd = b->getTilePosition().y + b->getType().tileHeight();

      // Buildings with addons
      if (b->getPlayer() == BWAPI::Broodwar->self() &&
              b->getType() == BWAPI::UnitTypes::Terran_Factory ||
          b->getType() == BWAPI::UnitTypes::Terran_Starport ||
          b->getType() == BWAPI::UnitTypes::Terran_Science_Facility ||
          b->getType() == BWAPI::UnitTypes::Terran_Command_Center)
        xEnd += 2;
  ...
  for (auto &o : buildingQueue.buildingsQueued()) {
    if (o.buildingType.isBuilding()) {
      unsigned x = o.designatedLocation.x,
               xEnd = o.designatedLocation.x + o.buildingType.tileWidth();
      unsigned y = o.designatedLocation.y,
               yEnd = o.designatedLocation.y + o.buildingType.tileHeight();

      // Buildings with addons
      if (o.buildingType == BWAPI::UnitTypes::Terran_Factory ||
          o.buildingType == BWAPI::UnitTypes::Terran_Starport ||
          o.buildingType == BWAPI::UnitTypes::Terran_Science_Facility ||
          o.buildingType == BWAPI::UnitTypes::Terran_Command_Center)
        xEnd += 2;
  ...
  for (int state = 0; !xMax || !xMin || !yMax || !yMin; ++state) {
    for (unsigned i = 0; i < e - 1; ++i) {
      if (canPlaceBuilding(ut, suggestedPosition) &&
          BWAPI::Broodwar->canBuildHere(suggestedPosition, ut, nullptr, false))
        return suggestedPosition;
...
bool BuildingPlacer::canPlaceBuilding(BWAPI::UnitType ut,
                                      BWAPI::TilePosition location) {
  BWAPI::TilePosition pos(location);
  int xEnd = location.x + ut.tileWidth(), yEnd = location.y + ut.tileHeight();

  // Buildings with addons
  if (ut == BWAPI::UnitTypes::Terran_Factory ||
      ut == BWAPI::UnitTypes::Terran_Starport ||
      ut == BWAPI::UnitTypes::Terran_Science_Facility)
    xEnd += 2;
