...
FastAPproximation::FAPUnit::FAPUnit(EnemyData ed)
    : x(ed.lastPosition.x), y(ed.lastPosition.y),
      ...
      health(ed.expectedHealth()), maxHealth(ed.lastType.maxHitPoints()),

      shields(ed.expectedShields()),
      shieldArmor(ed.lastPlayer->getUpgradeLevel(
          BWAPI::UpgradeTypes::Protoss_Plasma_Shields)),
      maxShields(ed.lastType.maxShields()),
      armor(ed.lastPlayer->armor(ed.lastType)), flying(ed.lastType.isFlyer()),
      ...
      unitType(ed.lastType), isOrganic(ed.lastType.isOrganic()),
      score(ed.lastType.destroyScore()), player(ed.lastPlayer) {
  ...
  case BWAPI::UnitTypes::Protoss_Carrier:
    ...
    if (ed.u && ed.u->isVisible()) {
      ...
      if (ed.lastPlayer) {
        groundCooldown =
            (int)round(37.0f / (ed.lastPlayer->getUpgradeLevel(
  ...
  case BWAPI::UnitTypes::Terran_Bunker:
    ...
    groundMaxRange = ed.lastPlayer->weaponMaxRange(
